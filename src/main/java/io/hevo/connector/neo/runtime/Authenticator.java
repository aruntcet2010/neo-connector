 ...
 * (sources/declarative/auth/token.py). P1 scope: ApiKeyAuthenticator (header or query parameter
 * injection), BearerAuthenticator, BasicHttpAuthenticator, NoAuth. OAuth lands in P3.
 */
  ...
  public static Authenticator from(Map<String, Object> component, Map<String, Object> config) {
    ...
    switch (type == null ? "NoAuth" : type) {
      ...
      case "BearerAuthenticator" -> {
        Authenticator auth = new Authenticator(config, false);
        String token = Components.getString(component, "api_token");
        auth.headers.put("Authorization", InterpolatedString.create("Bearer " + token, parameters));
        return auth;
      }
      ...
      default -> throw new ManifestException("Unsupported authenticator type for P1: " + type);
    }
  }
