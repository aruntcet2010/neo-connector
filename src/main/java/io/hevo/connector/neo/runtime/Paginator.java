...
/**
 * DefaultPaginator + pagination strategies, mirroring requesters/paginators/{default_paginator.py,
 * strategies/*.py}:
 *
 * <ul>
 ...
 *   <li>CursorPagination: token = interpolated {@code cursor_value} with {@code response}, {@code
 *       headers}, {@code last_record}, {@code last_page_size} in context; stops when the {@code
 *       stop_condition} is truthy or the cursor renders empty.
 * </ul>
 *
...
public final class Paginator {
  ...
  private static Strategy strategyFrom(Map<String, Object> component, Map<String, Object> config) {
    String type = Components.type(component);
    if (type == null) {
  ...
  public Map<String, Object> requestOptions(RequestOptionSpec.InjectInto target, Object token) {
    ...
    if (pageSizeOption != null
      ...
      options.put(String.valueOf(pageSizeOption.fieldName().eval(config)), strategy.pageSize());
    }
    return options;
  }
  ...
  static final class PageIncrement implements Strategy {
    ...
    PageIncrement(Map<String, Object> component, Map<String, Object> config) {
      ...
      this.injectOnFirstRequest = Boolean.TRUE.equals(component.get("inject_on_first_request"));
    }
    ...
    @Override
    public Object nextPageToken(PageContext context) {
      if ((pageSize != null && context.lastPageSize() < pageSize) || context.lastPageSize() == 0) {
        return null;
      }
  ...
  static final class OffsetIncrement implements Strategy {
    ...
    @Override
    public Integer pageSize() {
      return pageSizeTemplate == null ? null : evalPageSize(pageSizeTemplate.eval(config), config);
    }
    ...
    private Integer evaluatedPageSize(PageContext context) {
      ...
      Object evaluated =
          pageSizeTemplate.eval(config, Map.of("response", contextResponse(context)));
  ...
  static final class CursorPagination implements Strategy {
    ...
    public Object nextPageToken(PageContext context) {
      ...
      evalContext.put(
          "response", context.decodedResponse() == null ? Map.of() : context.decodedResponse());
    ...
    if (headers != null) {
      for (Map.Entry<String, List<String>> entry : headers.entrySet()) {
        ...
        flattened.put(
            ...
            values == null || values.isEmpty() ? "" : values.size() == 1 ? values.get(0) : values);
      }
    }
    return flattened;
  }
